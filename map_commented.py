...
def init():
    ...
    goal_y = (
        largeur - 20
    )  # the goal to reach is at the upper left of the map (y-coordinate)
...
class Car(Widget):
    ...
    def move(self, rotation):
        ...
        self.sensor1 = (
            Vector(30, 0).rotate(self.angle) + self.pos
        )  # updating the position of sensor 1
        self.sensor2 = (
            Vector(30, 0).rotate((self.angle + 30) % 360) + self.pos
        ...
        if (
            ...
            self.signal1 = 1.0  # sensor 1 detects full sand
        if (
            ...
            self.signal2 = 1.0  # sensor 2 detects full sand
        if (
            ...
            self.signal3 = 1.0  # sensor 3 detects full sand
...
class Game(Widget):
    ...
    def update(
        ...
        orientation = (
            Vector(*self.car.velocity).angle((xx, yy)) / 180.0
        ...
        if distance < 100:  # when the car reaches its goal
            goal_x = (
                self.width - goal_x
            )  # the goal becomes the bottom right corner of the map (the downtown), and vice versa (updating of the x-coordinate of the goal)
            goal_y = (
                self.height - goal_y
            )  # the goal becomes the bottom right corner of the map (the downtown), and vice versa (updating of the y-coordinate of the goal)
